...
public class ServicePrincipalTokenProvider : ITokenProvider
{
    ...
    {
        var credentialChain = new ChainedTokenCredential(new ManagedIdentityCredential(), new AzureCliCredential());
        ...
        var token = await credentialChain.GetTokenAsync(tokenRequestContext, cancellationToken).ConfigureAwait(false);
