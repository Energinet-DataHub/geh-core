...
internal sealed class ServiceBusQueueDeadLetterHealthCheck
    : AzureServiceBusHealthCheck<ServiceBusQueueDeadLetterHealthCheckOptions>, IHealthCheck
{
    internal ServiceBusQueueDeadLetterHealthCheck(
        ServiceBusQueueDeadLetterHealthCheckOptions options,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var properties = await managementClient.GetQueueRuntimePropertiesAsync(
                    Options.QueueName,
                    cancellationToken)
            ...
            if (properties.Value.DeadLetterMessageCount > 0)
            {
                return new HealthCheckResult(
                    context.Registration.FailureStatus,
                    $"Queue '{Options.QueueName}' has dead-letter messages.");
            }
